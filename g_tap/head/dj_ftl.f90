...
!  tapenade 3.x
!
!  Differentiation of dj in forward (tangent) mode:
!   variations   of useful results: dj
...
DOUBLE PRECISION FUNCTION g_DJ(i, j, k, spec, ismpl, dj)
  USE ARRAYS
...
& g_bety, g_betz, g_bet
  ...
  EXTERNAL g_POR, g_DISP, g_VY, g_VX, g_VZ
  DOUBLE PRECISION :: POR, DISP, VY, VX, &
& VZ
  DOUBLE PRECISION :: g_POR, g_DISP, g_VY, g_VX, g_VZ
  INTRINSIC SQRT
...
&     g_bet
    ...
    g_result1 = g_POR(i, j + 1, k, ismpl, result1)
    g_result2 = g_DISP(i, j + 1, k, ismpl, result2)
...
&     g_bet
    ...
    IF (summ .GT. 0.d0) THEN
      g_dj = 2.d0*(g_prod-prod*g_summ/summ)/summ
      dj = 2.d0*prod/summ
    ELSE
      g_dj = 0.D0
    END IF
