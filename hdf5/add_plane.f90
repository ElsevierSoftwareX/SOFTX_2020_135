!>    @brief writes/add a additional plane (2D) to a hdf5-file
!>    @param[in] NI resulting array dimension
!>    @param[in] NJ resulting array dimension
!>    @param[in] NK new plane index
!>    @param[in] A new plane, [dimension] < [NI,NJ]
!>    @param[in] A_name array name (ascii)
!>    @param[in] f_name hdf5 file name
      SUBROUTINE add_plane(ni,nj,nk,a,a_name,f_name)
#ifndef noHDF
        USE hdf5
...
!      Data buffers
        ...
        IF (nk==1) THEN
          CALL write_hdf5(ni,nj,nk,a,a_name,f_name)
        ELSE IF (nk>1) THEN
          dims(1) = ni
          dims(2) = nj
          dims(3) = nk
