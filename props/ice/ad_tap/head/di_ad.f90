!        Generated by TAPENADE     (INRIA, Ecuador team)
!  tapenade 3.x
!
!  Differentiation of di in reverse (adjoint) mode (with options noISIZE i8):
!   gradient     of useful results: *temp *head *propunit *pres
!                di
!   with respect to varying inputs: *temp *head *propunit *pres
!   Plus diff mem management of: temp:in head:in propunit:in pres:in
!>    @brief average effective diffusivities on cell faces in x direction
!>    @param[in] i grid indices
!>    @param[in] j grid indices
!>    @param[in] k grid indices
!>    @param[in] spec species
!>    @param[in] ismpl local sample index
!>    @return effective diffusivities (J/mK)
SUBROUTINE DI_AD(i, j, k, spec, ismpl, di_adv)
  use arrays

  USE ARRAYS_AD

  USE MOD_GENRL
  IMPLICIT NONE
  double precision :: di_adv
  INTEGER :: ismpl
  INTEGER :: i, j, k
  INTEGER :: spec
  DOUBLE PRECISION :: f1, f2, prod, summ, betx, bety, betz, bet
  DOUBLE PRECISION :: f1_ad, f2_ad, prod_ad, summ_ad, betx_ad, bety_ad, &
& betz_ad, bet_ad
  EXTERNAL POR, DISP, VX, VY, VZ
  EXTERNAL POR_AD0, DISP_AD, VX_AD, VY_AD, VZ_AD
  DOUBLE PRECISION :: POR, DISP, VX, VY, &
& VZ
  INTRINSIC SQRT
  DOUBLE PRECISION :: result1
  DOUBLE PRECISION :: result1_ad
  DOUBLE PRECISION :: result2
  DOUBLE PRECISION :: result2_ad
  INTEGER :: arg1
  INTEGER :: arg2
  DOUBLE PRECISION :: temporary_ad
  INTEGER :: branch
  DOUBLE PRECISION :: di
  bety = 0.d0
  betz = 0.d0
  IF (k0 .GT. 1 .AND. k .LT. k0) THEN
    betz = VZ(i, j, k, ismpl)
    CALL PUSHREAL8(betz)
    betz = betz*betz
    CALL PUSHCONTROL1B(0)
  ELSE
    CALL PUSHCONTROL1B(1)
  END IF
  IF (j0 .GT. 1 .AND. j .LT. j0) THEN
    bety = VY(i, j, k, ismpl)
    CALL PUSHREAL8(bety)
    bety = bety*bety
    CALL PUSHCONTROL1B(0)
  ELSE
    CALL PUSHCONTROL1B(1)
  END IF
  IF (i0 .GT. 1 .AND. i .LT. i0) THEN
    betx = VX(i, j, k, ismpl)
    CALL PUSHREAL8(betx)
    betx = betx*betx
    bet = SQRT(betx + bety + betz)
    result1 = POR(i, j, k, ismpl)
    result2 = DISP(i, j, k, ismpl)
    f1 = result1*diff_c(spec) + result2*bet
    arg1 = i + 1
    result1 = POR(arg1, j, k, ismpl)
    arg2 = i + 1
    CALL PUSHREAL8(result2)
    result2 = DISP(arg2, j, k, ismpl)
    f2 = result1*diff_c(spec) + result2*bet
    prod = f1*f2
    summ = f1*delx(i+1) + f2*delx(i)
    IF (summ .GT. 0.d0) THEN
      temporary_ad = 2.d0*di_adv/summ
      prod_ad = temporary_ad
      summ_ad = -(prod*temporary_ad/summ)
    ELSE
      prod_ad = 0.D0
      summ_ad = 0.D0
    END IF
    f1_ad = delx(i+1)*summ_ad + f2*prod_ad
    f2_ad = delx(i)*summ_ad + f1*prod_ad
    result1_ad = diff_c(spec)*f2_ad
    result2_ad = bet*f2_ad
    bet_ad = result2*f2_ad
    CALL POPREAL8(result2)
    CALL DISP_AD(arg2, j, k, ismpl, result2_ad)
    CALL POR_AD0(arg1, j, k, ismpl, result1_ad)
    result1_ad = diff_c(spec)*f1_ad
    result2_ad = bet*f1_ad
    bet_ad = bet_ad + result2*f1_ad
    CALL DISP_AD(i, j, k, ismpl, result2_ad)
    CALL POR_AD0(i, j, k, ismpl, result1_ad)
    IF (betx + bety + betz .EQ. 0.0) THEN
      temporary_ad = 0.D0
    ELSE
      temporary_ad = bet_ad/(2.0*SQRT(betx+bety+betz))
    END IF
    betx_ad = temporary_ad
    bety_ad = temporary_ad
    betz_ad = temporary_ad
    CALL POPREAL8(betx)
    betx_ad = 2*betx*betx_ad
    CALL VX_AD(i, j, k, ismpl, betx_ad)
  ELSE
    bety_ad = 0.D0
    betz_ad = 0.D0
  END IF
  CALL POPCONTROL1B(branch)
  IF (branch .EQ. 0) THEN
    CALL POPREAL8(bety)
    bety_ad = 2*bety*bety_ad
    CALL VY_AD(i, j, k, ismpl, bety_ad)
  END IF
  CALL POPCONTROL1B(branch)
  IF (branch .EQ. 0) THEN
    CALL POPREAL8(betz)
    betz_ad = 2*betz*betz_ad
    CALL VZ_AD(i, j, k, ismpl, betz_ad)
  END IF
END SUBROUTINE DI_AD

