...
!>that this notice and the above copyright notice remain intact.       %\n
      double precision function backtr(vrgs,nt,vr,vrg,zmin,zmax,ltail,
     +                     ltpar,utail,utpar)
c-----------------------------------------------------------------------
c
...
c-----------------------------------------------------------------------
      ...
      double precision ltpar,utpar,lambda
      integer   ltail,utail
c
...
c
      if(vrgs.le.vrg(1)) then
            ...
            cdfbt  = gcum(vrgs)
            if(ltail.eq.1) then
                  backtr = powint(0.0d0,cdflo,zmin,vr(1),cdfbt,1.0d0)
            else if(ltail.eq.2) then
                  cpow   = 1.0 / ltpar
                  backtr = powint(0.0d0,cdflo,zmin,vr(1),cdfbt,cpow)
            endif
c
...
c
            ...
            cdfbt  = gcum(vrgs)
            if(utail.eq.1) then
                  backtr = powint(cdfhi,1.0d0,vr(nt),zmax,cdfbt,1.0d0)
            else if(utail.eq.2) then
                  cpow   = 1.0d0 / utpar
                  backtr = powint(cdfhi,1.0d0,vr(nt),zmax,cdfbt,cpow)
            else if(utail.eq.4) then
                  lambda = (vr(nt)**utpar)*(1.0d0-gcum(vrg(nt)))
                  backtr = (lambda/(1.0d0-gcum(vrgs)))**(1.0d0/utpar)
            endif
      else
c
...
c
            ...
            j = max(min((nt-1),j),1)
            backtr = powint(vrg(j),vrg(j+1),vr(j),vr(j+1),vrgs,1.0d0)
      endif
      return
      end
