...
!>that this notice and the above copyright notice remain intact.       %\n
      double precision function getz(pval,nt,vr,cdf,zmin,zmax,ltail,
     +                   ltpar,utail,utpar)
c-----------------------------------------------------------------------
c
...
c
      if(pval.le.cdf(1)) then
                  getz = vr(1)
            if(ltail.eq.1) then
                  getz = powint(0.0d0,cdf(1),zmin,vr(1),pval,1.0d0)
            else if(ltail.eq.2) then
                  cpow = 1.0d0 / ltpar
                  getz = powint(0.0d0,cdf(1),zmin,vr(1),pval,cpow)
            endif
c
...
c
            ...
            if(utail.eq.1) then
                  ...
                  getz   = (lambda/(1.0d0-pval))**(1.0d0/utpar)
            endif
      else
c
