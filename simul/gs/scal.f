...
!>that this notice and the above copyright notice remain intact.       %\n
        subroutine scal(lxmin,lxmax,lymin,lymax,xaxmin,xaxmax,yaxmin,
     +                  yaxmax,ilog,i45)
c-----------------------------------------------------------------------
c
...
c
c INPUT VARIABLES:
c       lxmin   - the minimum of the x axis (labeled on axis)
c       lxmax   - the maximum of the x axis (labeled on axis)
c       lymin   - the minimum of the y axis (labeled on axis)
c       lymax   - the maximum of the y axis (labeled on axis)
...
c-----------------------------------------------------------------------
      ...
      parameter (EPS=0.001)
      double precision xloc(5),yloc(5), lxmin,lxmax,lymin,lymax
      character (len=8) :: label
      character (len=8) :: lfmt
      intrinsic dlog10
c
...
c
      if((lxmax-lxmin).le.0.0001.or.(lymax-lymin).le.0.0001) return
c
...
c
      if(i45.eq.1) then
         if(abs(lxmin-lymin).le.0.0001.and.
     &      abs(lxmax-lymax).le.0.0001) then
            xloc(1) = xaxmin
            yloc(1) = yaxmin
            xloc(2) = xaxmax
            yloc(2) = yaxmax
...
c
            ...
            do i=1,100
                  yloc(2) = yaxmin - tlng
                  xloc(1) = resc(lxmin,lxmax,xaxmin,xaxmax,pos)
                  xloc(2) = resc(lxmin,lxmax,xaxmin,xaxmax,pos)
                  call psline(num,xloc,yloc,pl2,0)
                  yloc(2) = yloc(1) - 2.5*tlng
...
c
            ...
            do i=1,100
                  xloc(2) = xaxmin - tlng
                  yloc(1) = resc(lymin,lymax,yaxmin,yaxmax,pos)
                  yloc(2) = resc(lymin,lymax,yaxmin,yaxmax,pos)
                  call psline(num,xloc,yloc,pl2,0)
