...
c-----------------------------------------------------------------------
      ...
      include 'gslib.inc'
c
      double precision var(50), aa1,aa2,av, c,d,e,f,g,h
      double precision p,acorni,acorni2,cp,oldcp,w, vvreal, xx
      external acorni,acorni2
      character (len=80) :: cdfl
      character (len=80) :: tmpfl
      character (len=40) :: datafl
      character (len=40) :: btfl
      character (len=40) :: dbgfl
      character (len=40) :: lvmfl
      character (len=40) :: str
      character (len=40) :: parfile
      character (len=40) :: volgeomfl
      character (len=40) :: volsumfl
      character (len=40) :: datacovfl
      logical   testfl
      integer :: i,j,k,idata,idum,isecvr,ivrl,iwt,lindex, nvarbt
      integer :: ix,iy,iz, ix1,iy1,iz1, ixl,iyl,izl, nmonte,nt,nvari
      integer :: ndinv,ivol,iv,ivol_new,nvoldata,IntVar
      double precision ivolx,ivoly,ivolz,ivoll,cumsum,vvtmp,realtmp
      double precision radius1,radius2,sumwt, sill,ss,twt
      logical   testind 
!      integer    iargc,argc
!      external iargc
      integer :: tempa,tempb, realz,imodel
c
...
c      read(lin,*,err=98) idbg,read_covtable
      ...
      if (idbg.gt.-2) then
         write(*,9999) VERSION
 9999    format(/' VISIM Version: ',f6.4,' Start')
      endif
...
c      open(ldbg,file=dbgfl,status='UNKNOWN')
      ...
      write(outfl,'(1A,1I4.4,1A)') 'sim',realz,'.dat'
...
c
     ...
     1           '      only allow 1,2 or 4 - see manual '
        ...
        if(utail.eq.4.and.utpar.lt.1.0d0) then
            if (idbg.gt.-1) write(*,*) 
     ...
     1           '      must be greater than 1.0!'
        ...
        if(ltail.eq.2.and.ltpar.lt.0.0d0) then
            if (idbg.gt.-1) write(*,*) 
     ...
     1           '      must be greater than 0.0!'
        ...
        if(utail.eq.2.and.utpar.lt.0.0d0) then
            if (idbg.gt.-1) write(*,*) 
...
c variable values at each grid node. 
 ...
 104        format('WARNING secondary attribute file ',a40,
     +           ' does not exist!')
         ...
         av = av / max(dble(nxyz),1.0d0)
         ss =(ss / max(dble(nxyz),1.0d0)) - av * av
