...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      include 'visim.inc'
c
      integer :: id,id2, ind,in,irepo,infoct,is,itt, iz,iy,ix
      integer :: lktype,lindex, nsec,nsbtosr, nxsup,nysup,nzsup
      double precision acorni, gmean,cmean,cstdev, sec2,sec3, TINY
      double precision zsizsup,ysizsup,xsizsup, test,test2, xx,yy,zz
      double precision xmnsup,ymnsup,zmnsup
      external acorni
      logical testind
...
c
                  if(sstrat.eq.0.and.test.le.TINY) sim(ind)=10.0d0*UNEST
            end do
c
...
c
	       ...
	       write(ldbg, *) 'Sample data location and grid search'    
            ...
            do in=1,nxyz
                  lindex = int(in+0.5d0)
	          if (idbg.ge.3) then 
                    WRITE(ldbg, *) 'SIM(', lindex, ')=', SIM(lindex)
		  end if 
c
c (sim(lindex).gt.(UNEST+EPSLON)) is satisfied when data are assigned to
c the nearest grid node as specified by sstrat=1 option. 
c (sim(lindex).lt.(UNEST*2.0)) is satisfied when data is too close to grid
c node in the case when data are not assign to the grid node(sstrat=0)
c

                  if(sim(lindex).gt.(UNEST+EPSLON).or.
     +               sim(lindex).lt.(UNEST*2.0d0)) then
                     krgvar(lindex) = 0.0d0
                     go to 5
                  end if 
...
c     
     ...
     +                    gmean,cmean,cstdev)
                     ...
                     krgvar(lindex) = cstdev * cstdev 
                   if (krgvar(lindex).ge.zmaxvar) zmaxvar=krgvar(lindex)
 5                   continue
                  end do 
c     
c     MAIN LOOP OVER ALL THE NODES TO GET THE KRIGING VARIANCE:
