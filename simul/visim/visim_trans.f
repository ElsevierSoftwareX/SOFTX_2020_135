...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit none
      include 'visim.inc'
      double precision var(50),c,d,e,f,g,h, cp,evmax,gmedian,oldcp
      double precision tcdf, av, powint
      external powint
      integer :: i,j,nvari
...
c     
      ...
      if(ncut.le.1.or.tcdf.le.EPSLON) then
         ...
         if(utail.eq.4.and.rvr(ncut).le.0.0d0) then
...
c     
      call locate(rcdf,ncut,1,ncut,0.5d0,j)
      gmedian = powint(rcdf(j),rcdf(j+1),rvr(j),rvr(j+1),0.5d0,1.0d0)
...
c-----------------------------------------------------------------------
      ...
      include 'visim.inc'
c 
      double precision d,e,f,g,h, cp,oldcp, getz
      external getz
      double precision tcdf, av, ss, wtw, zval, evmax
      integer :: i,j, ne, num, nvarik
      character (len=40) :: str
...
c     
            ...
            do i=1,num
               cp     = cp + dcdf(i)*tcdf     
               dcdf(i) =(cp + oldcp) /2.d0
               if(dcdf(i).ge.1) dcdf(i) = 0.99d0
...
c     9998 format(/' TRANS Version: ',f6.4, ' Finished'/)
c
            return
